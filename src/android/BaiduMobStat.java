...
public class BaiduMobStat extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if ("start".equals(action)) {
            StatService.start(cordova.getActivity());
            // StatService.setSendLogStrategy(cordova.getActivity(), SendStrategyEnum.APP_START,1,false);
        }
        else {
            return false;
        }
    ...
    //     if("start".equals(action)){
    //         // StatService.start(cordova.getActivity());
    //         StatService.setSendLogStrategy(webView.getContext(), SendStrategyEnum.APP_START,1,false);
